...
DEFAULT_NVMF_TGT_CONF = '/etc/spdk/vhost.conf'
...
def do_config(args):
    ...
    for line in nvme_list:
        ...
        if m:
            pciaddr = m.group(1)
            print("do_config: unbind_nvme_dev %s" % pciaddr)
...
def find_iotdma():
    ...
    global iotdma_ids
    stdout = subprocess.check_output(['/usr/sbin/lspci', '-D', '-v', '-d::880'])
...
def load_pci_ids():
    global pci_ids
    # 0000:00:04.0 0880: 8086:6f20 (rev 01)
    # OR
    # 0000:8f:00.2 0108: 15b3:6001
    # , '-d::108'
    stdout = subprocess.check_output(['/usr/sbin/lspci', '-D', '-n'])
    rx = re.compile('^(\S+)\s(\w+):\s(\w+):(\w+)')
    for line in stdout.splitlines():
       m = rx.search(line)
       if not m:
           continue
       bdf, cls, ven_id, dev_id = map(lambda i: m.group(i), (1,2,3,4))
       if cls not in ('0880', '0108'):
           continue
...
if __name__ == '__main__':
    parser = ArgumentParser(prog='setup_vhost')
